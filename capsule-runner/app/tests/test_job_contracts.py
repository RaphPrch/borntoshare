...
def test_job_contracts_keep_actor_and_default_action_consistent() -> None:
    ...
    assert default_action_for_job_type("LDAP_TEST") == "test_ldap"
    assert default_action_for_job_type("SMB_PROBE") == "test_smb_ntlm"
    assert queue_job_types_csv() == ",".join(sorted(JOB_CONTRACTS))


def test_job_contracts_reject_unknown_job_type() -> None:
    with pytest.raises(ValueError, match="Unsupported job_type: UNKNOWN_JOB"):
        default_action_for_job_type("UNKNOWN_JOB")
